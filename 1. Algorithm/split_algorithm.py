...
def estimate_guassians_on_manifold(once,twice, Model_1, Model_2, once_data, twice_data, window_3d, split, final_window, window_step_size, data_output_gibbs_ORIGNAL,data_output_gibbs_PROJECTED, first_time_association, once_data_ORIGNAL, once_data_PROJECTED, twice_data_ORIGNAL, twice_data_PROJECTED, Lineage_1, Lineage_2, previouse_window_L1, previouse_window_L2, previouse_window_L1_PROJECTED,previouse_window_L2_PROJECTED, Model_1_Lineage_1_counter, Model_1_Lineage_2_counter):
    ...
    if (split == False):
        if (once == True) and (twice == False) and (Model_1 == True):
            ...
            # once_data_ORIGNAL
            once, twice,Lineage_1, Lineage_2, previouse_window_L1, previouse_window_L2,previouse_window_L1_PROJECTED,previouse_window_L2_PROJECTED, first_time_association,  Model_1_Lineage_1_counter, Model_1_Lineage_2_counter = association(once, twice, split,window_3d, Model_1, Model_2, Lineage_1, Lineage_2, data_output_gibbs_ORIGNAL, data_output_gibbs_PROJECTED, once_data, once_data_PROJECTED, twice_data, twice_data_PROJECTED, first_time_association, previouse_window_L1, previouse_window_L2, previouse_window_L1_PROJECTED,previouse_window_L2_PROJECTED, Model_1_Lineage_1_counter, Model_1_Lineage_2_counter)
            #Reset Flags
            once = False
            twice = False 
    ...
    elif (Model_2 == True) and (once == True) and (twice == True) and (first_time_association == False):
    ...
    return once, twice, split, once_data, twice_data, once_data_ORIGNAL, once_data_PROJECTED, twice_data_ORIGNAL, twice_data_PROJECTED, first_time_association, Lineage_1, Lineage_2



def association(once, twice, split,before_split, Model_1, Model_2, Lineage_1, Lineage_2, data_output_gibbs_ORIGNAL, data_output_gibbs_PROJECTED, once_data_ORIGNAL, once_data_PROJECTED, twice_data_ORIGNAL, twice_data_PROJECTED, first_time_association, previouse_window_L1, previouse_window_L2 ,previouse_window_L1_PROJECTED,previouse_window_L2_PROJECTED, Model_1_Lineage_1_counter, Model_1_Lineage_2_counter):
    #There is no split in data
    if (split == False) and (Model_1 == True) and (once == False) and (twice == False):
        #Lineage 1
        before_split.columns = [ 'Pseudo_Time_normal','tsne_1','tsne_2']    #Set to lineage formating
        ...
        before_split.columns = [ 'G1_Pseudo_Time_normal','G1_tsne_1','G1_tsne_2'] #Reset to orignal formating4
        ...
        frames = [Lineage_2, before_split]
        Lineage_2 = pd.concat(frames, axis=0, ignore_index=False, sort=True)
        before_split.columns = [ 'G1_Pseudo_Time_normal','G1_tsne_1','G1_tsne_2'] #Reset to orignal formating
        ...
        if (first_time_association == True):
            ...
            d = {'tsne_1':once_data_PROJECTED['G1_tsne_1'],'tsne_2':once_data_PROJECTED['G1_tsne_2']}
            current_PROJECTED_window_G1 = pd.DataFrame(d)
            current_PROJECTED_window_G1 = current_PROJECTED_window_G1.dropna() #Drop NAN rows

            d = {'tsne_1':once_data_PROJECTED['G2_tsne_1'],'tsne_2':once_data_PROJECTED['G2_tsne_2']}
            ...
            d = {'Pseudo_Time_normal':once_data_ORIGNAL['G1_Pseudo_Time_normal'],'tsne_1':once_data_ORIGNAL['G1_tsne_1'],'tsne_2':once_data_ORIGNAL['G1_tsne_2']}
            current_ORIGINAL_window_G1_3d = pd.DataFrame(d)
            current_ORIGINAL_window_G1_3d = current_ORIGINAL_window_G1_3d.dropna() #Drop NAN rows

            d = {'Pseudo_Time_normal':once_data_ORIGNAL['G2_Pseudo_Time_normal'],'tsne_1':once_data_ORIGNAL['G2_tsne_1'],'tsne_2':once_data_ORIGNAL['G2_tsne_2']}
            current_ORIGINAL_window_G2_3d = pd.DataFrame(d)
            current_ORIGINAL_window_G2_3d = current_ORIGINAL_window_G2_3d.dropna()#Drop NAN rows

            #Data in original 2d window
            d = {'tsne_1':once_data_ORIGNAL['G1_tsne_1'],'tsne_2':once_data_ORIGNAL['G1_tsne_2']}
            current_ORIGINAL_window_G1_2d = pd.DataFrame(d)
            current_ORIGINAL_window_G1_2d = current_ORIGINAL_window_G1_2d.dropna() #Drop NAN rows

            d = {'tsne_1':once_data_ORIGNAL['G2_tsne_1'],'tsne_2':once_data_ORIGNAL['G2_tsne_2']}
            current_ORIGINAL_window_G2_2d = pd.DataFrame(d)
            current_ORIGINAL_window_G2_2d = current_ORIGINAL_window_G2_2d.dropna()#Drop NAN rows

            #Assign NEW previouse windows
            previouse_window_L1 = current_ORIGINAL_window_G1_3d#current_PROJECTED_window_G1
            previouse_window_L2 = current_ORIGINAL_window_G2_3d#current_PROJECTED_window_G2
            ...
            '''
            if (twice == True):
                Lineage_1, Lineage_2, previouse_window_L1, previouse_window_L2, previouse_window_L1_PROJECTED,previouse_window_L2_PROJECTED, Model_1_Lineage_1_counter, Model_1_Lineage_2_counter = euclidean_dist_association(previouse_window_L1, previouse_window_L2,previouse_window_L1_PROJECTED, previouse_window_L2_PROJECTED, twice_data_ORIGNAL,twice_data_PROJECTED, Lineage_1 , Lineage_2, Model_1, Model_2,Model_1_Lineage_1_counter, Model_1_Lineage_2_counter)   
            '''
            -----------------------------------------------------------------
            Current
            -----------------------------------------------------------------
            ...
            if (once == True) and (twice == True):
                Lineage_1, Lineage_2,  previouse_window_L1, previouse_window_L2, previouse_window_L1_PROJECTED,previouse_window_L2_PROJECTED, Model_1_Lineage_1_counter, Model_1_Lineage_2_counter = euclidean_dist_association(previouse_window_L1, previouse_window_L2,previouse_window_L1_PROJECTED, previouse_window_L2_PROJECTED, data_output_gibbs_ORIGNAL,data_output_gibbs_PROJECTED, Lineage_1 , Lineage_2, Model_1, Model_2,Model_1_Lineage_1_counter, Model_1_Lineage_2_counter)
    ...
    return once, twice,Lineage_1, Lineage_2, previouse_window_L1, previouse_window_L2 ,previouse_window_L1_PROJECTED,previouse_window_L2_PROJECTED, first_time_association,  Model_1_Lineage_1_counter, Model_1_Lineage_2_counter
...
def AFTER_split_euclidean_dist_association(Lineage_1, Lineage_2,window_3d):
    ...
    L1_counter = 0
    L2_counter = 0
    for l in tqdm(range(len(temp_ID_WINDOW.index)), desc ="Data association after split."):
        ...
        temp_ID_WINDOW = temp_ID_WINDOW[~temp_ID_WINDOW['cell_ID_number'].isin(select_one['cell_ID_number'].values)]
...
def euclidean_dist_association(previouse_window_L1, previouse_window_L2,previouse_window_L1_PROJECTED, previouse_window_L2_PROJECTED, data_output_gibbs_ORIGNAL,data_output_gibbs_PROJECTED, Lineage_1 , Lineage_2, Model_1, Model_2,Model_1_Lineage_1_counter, Model_1_Lineage_2_counter):
    ...
    #Data projected onto plane
    d = {'tsne_1':data_output_gibbs_PROJECTED['G1_tsne_1'],'tsne_2':data_output_gibbs_PROJECTED['G1_tsne_2']}
    current_PROJECTED_window_G1 = pd.DataFrame(d)
    current_PROJECTED_window_G1 = current_PROJECTED_window_G1.dropna() #Drop NAN rows

    d = {'tsne_1':data_output_gibbs_PROJECTED['G2_tsne_1'],'tsne_2':data_output_gibbs_PROJECTED['G2_tsne_2']}
    current_PROJECTED_window_G2 = pd.DataFrame(d)
    current_PROJECTED_window_G2 = current_PROJECTED_window_G2.dropna() #Drop NAN rows

    #Data in original 3d window
    d = {'Pseudo_Time_normal':data_output_gibbs_ORIGNAL['G1_Pseudo_Time_normal'],'tsne_1':data_output_gibbs_ORIGNAL['G1_tsne_1'],'tsne_2':data_output_gibbs_ORIGNAL['G1_tsne_2']}
    current_ORIGINAL_window_G1_3d = pd.DataFrame(d)
    current_ORIGINAL_window_G1_3d = current_ORIGINAL_window_G1_3d.dropna() #Drop NAN rows

    d = {'Pseudo_Time_normal':data_output_gibbs_ORIGNAL['G2_Pseudo_Time_normal'],'tsne_1':data_output_gibbs_ORIGNAL['G2_tsne_1'],'tsne_2':data_output_gibbs_ORIGNAL['G2_tsne_2']}
    current_ORIGINAL_window_G2_3d = pd.DataFrame(d)
    current_ORIGINAL_window_G2_3d = current_ORIGINAL_window_G2_3d.dropna() #Drop NAN rows

    d = {'tsne_1':data_output_gibbs_ORIGNAL['G1_tsne_1'],'tsne_2':data_output_gibbs_ORIGNAL['G1_tsne_2']}
    current_ORIGINAL_window_G1 = pd.DataFrame(d)
    current_ORIGINAL_window_G1_2d = current_ORIGINAL_window_G1.dropna() #Drop NAN rows

    d = {'tsne_1':data_output_gibbs_ORIGNAL['G2_tsne_1'],'tsne_2':data_output_gibbs_ORIGNAL['G2_tsne_2']}
    current_ORIGINAL_window_G2_2d = pd.DataFrame(d)
    current_ORIGINAL_window_G2_2d = current_ORIGINAL_window_G2_2d.dropna() #Drop NAN rows
    ...
    if len(previouse_window_L1.index) > len(previouse_window_L2.index):
        previouse_window_L1 = previouse_window_L1.sample(n = len(previouse_window_L2.index)) 
    else:
        previouse_window_L2 = previouse_window_L2.sample(n = len(previouse_window_L1.index))
    ...
    #Calculate euclidean distance
    euclidean_distance_L1_G1 = (current_ORIGINAL_window_G1_3d - mean).pow(2).sum(1).pow(0.5)
    euclidean_distance_L1_G1 = euclidean_distance_L1_G1.mean(axis=0)

    mean = previouse_window_L2.mean(axis=0)
    mean.columns = ['tsne_1','tsne_2']
    euclidean_distance_L2_G1 = (current_ORIGINAL_window_G1_3d - mean).pow(2).sum(1).pow(0.5)
    ...
    return Lineage_1, Lineage_2, NEW_previouse_window_L1, NEW_previouse_window_L2, NEW_previouse_window_L1_PROJECTED, NEW_previouse_window_L2_PROJECTED, Model_1_Lineage_1_counter, Model_1_Lineage_2_counter


def bifurcation_points(split, bifurcation_data, bifurcate_once,total_bifurcations, once_data):
    if (split == True) and (bifurcate_once == False):
        ...
        if bifurcation_data.empty:
            ...
            bifurcation_data = pd.concat(frames, axis=1, sort=False)#Append new bifurcation data
            total_bifurcations= total_bifurcations +1
